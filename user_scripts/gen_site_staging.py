...
def get_module_list(page_id: str) -> List[str]:
    """获取指定页面的模块列表"""
    query = f"select module_id from staging_site_page_module where page_id='{page_id}' order by priority"
...
def get_module_items(page_id: str, module_id: str) -> List[Dict]:
    """获取指定页面和模块的内容项列表"""
    query = f"select * from staging_site_page_module_item where page_id='{page_id}' and module_id='{module_id}' and parent_key='' order by priority"
    result_list = execute_query(query)
    return result_list
...
def get_module_items_by_parent(page_id: str, module_id: str, parent_key: str) -> List[Dict]:
    """获取指定页面和模块的内容项列表"""
    query = f"select * from staging_site_page_module_item where page_id='{page_id}' and module_id='{module_id}' and parent_key='{parent_key}' order by priority"
    result_list = execute_query(query)
    return result_list
...
def parse_img(item: Dict) -> Dict:
    ...
    res['_track'] = {
        'itemId': uuid1,
        'itemName': item['title'] if item['title'] != '' else uuid1
    }
...
def get_page_config(page_id, page_title):
    modules = execute_query(f'select * from staging_site_page_module where page_id = \'{page_id}\' order by priority ')
    module_config_list = []
    module_ids = []
